...
mmap(uint addr, int length, int prot, int flags, int fd, int offset) {
  ...
  if (BITCHECK(flags, 2)) { // MAP_POPULATE is given.
    for (int j = length / PGSIZE, i = 0; i < j; i++) {
      int m_id;
      void* physical_page;
      if ((physical_page = kalloc()) < 0) return 0;            //return 0: kalloc function fail
      if ((fileread(f, physical_page, PGSIZE)) < 0) return 0;  //return 0: file functions fail
      if ((mmap_helper(p, vaddr + i * PGSIZE, PGSIZE, physical_page, prot)) < 0) return 0;
  ...
  // if (!(BITCHECK(flags, 2))) return vaddr; //If MAP_POPULATE is not given, just record its mapping area.
